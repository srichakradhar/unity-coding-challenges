...
namespace UniCode
{
    public class ConcatenationOfArray
    {
        ...
        {
            return new int[] {}; // nums.Concat(nums).ToArray()
        }

        [MenuItem("Component/UniCode/Arrays/ConcatenationOfArray")]
        static void run()
        {
            Debug.Assert(Enumerable.SequenceEqual(
                    GetConcatenation(new int[] { 1, 2, 1 }),
                    new int[] { 1, 2, 1, 1, 2, 1 }), "Wrong Answer!");
            Debug.Assert(Enumerable.SequenceEqual(
                    GetConcatenation(new int[] { 1, 3, 2, 1 }),
                    new int[] { 1, 3, 2, 1, 1, 3, 2, 1 }), "Wrong Answer!");
